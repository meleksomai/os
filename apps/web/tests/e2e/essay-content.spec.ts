...
test("custom MDX components render (quotes)", async ({ page }) => {
  await page.goto("/essay/agents");

  // The agents essay imports the Quote block from @workspace/ui, which renders
  // a figure with the quoted text and its attribution.
  const quote = page.locator(".prose figure blockquote").first();
  await expect(quote).toBeAttached();
  await expect(page.locator(".prose figure figcaption").first()).toContainText(
    /\S/
  );
});
