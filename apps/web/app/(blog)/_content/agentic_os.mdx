...
Over the past year, I have become more and more convinced that the future of softwares lies in _agent-first_ systems powered by Frontier Language Models (LLMs) or whatever comes next.[^1] By agent-first systems, I mean software systems in which the task to be performed is further hidden from the end-user. The task itself is abstracted awau fro the user. In other words, the agent manages the intent interpretation, memory, decision-making, action execution, and ultimately _removes_ further the user from the task. This is analogous - _to an extend_ - to the way modern high-level programming languages such as Python, Javascript, and others abstract away garbage collection, memory allocation, etc. However, the similarities ends here. LLMs pushes the boundary to perform more tasks that involve "thinking", "conceptualization", and ultimately "craft". 

Furthermore, LLMs seems to have a very different user interaction (UX/UI) from traditional software. In _traditional_[^2] softwares the user is expected to interact with a specific UI to accomplish a task. They have to navigate menus, click on buttons, and fill out forms. Designers would create incredible [design](https://www.radix-ui.com) [systems](https://mui.com/material-ui/) to make the user experience suck less. With LLMs, the user no longer needs to learn how to use a piece of software; instead, they can rely on the agent to understand their intent and use the appropriate tools to achieve their goals. 
