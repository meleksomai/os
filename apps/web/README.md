...
| --- | --- | --- | --- |
| Unit | `tests/unit/**/*.test.ts(x)` (Vitest), `packages/emailing/*.test.ts` | SDK mocked | every PR (`pnpm test`) |
...
| Contract | `packages/emailing/contract/` (the `contract` Vitest project) | the real API, with a dedicated Resend audience | scheduled `contract` workflow, or `pnpm --filter @workspace/emailing test:contract` |
